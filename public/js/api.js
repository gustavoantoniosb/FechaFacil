...
const API = {
  ...
  async _rest(method, path, { body, single } = {}) {
    ...
    if (!res.ok) throw new Error(data.message || data.hint || data.details || 'Erro');
    return data;
  },
  ...
  async _auth(path, body) {
    ...
    if (!res.ok) throw new Error(d.error_description || d.msg || d.error || 'Erro de autenticacao');
    return d;
  },
  ...
  async updateMe(data) {
    const profile = await this._rest('PATCH', `profiles?id=eq.${this.userId()}`, { body: data, single: true });
    const cached = JSON.parse(localStorage.getItem('ff_user') || '{}');
  ...
  async getClients() {
    const rows = await this._rest('GET', `clients?user_id=eq.${this.userId()}&select=*,quotes(id,total,created_at)&order=created_at.desc`);
    return rows.map(c => {
  ...
  async createQuote({ client_name, client_phone, client_email, items, notes, valid_until, discount = 0 }) {
    ...
    const quote = await API._rest('POST', 'quotes', {
      body: { user_id: API.userId(), client_name, client_phone, client_email, notes, valid_until, discount, total }, single: true
    });
    if (items.length) {
      await API._rest('POST', 'quote_items', {
        body: items.map(i => ({ quote_id: quote.id, description: i.description, quantity: Number(i.quantity)||1, unit_price: Number(i.unit_price), total: (Number(i.quantity)||1)*Number(i.unit_price) }))
      });
    }
    return quote;
  },
  ...
  async dashboard() {
    ...
    return { month_revenue, month_approved, month_quotes, conversion, pending_quotes, total_clients: clients.length, recent_quotes: allQuotes.slice(0, 5) };
  },

  async getPublicQuote(token) {
    const rows = await fetch(`${SUPA_URL}/rest/v1/quotes?token=eq.${token}&select=*,quote_items(*)`, { headers: { 'apikey': SUPA_KEY } }).then(r => r.json());
    if (!rows?.length) throw new Error('Orcamento nao encontrado');
    return { ...rows[0], items: rows[0].quote_items || [] };
  },

  approveQuote(token, approved_by) {
    return fetch(`${SUPA_URL}/rest/v1/quotes?token=eq.${token}`, {
      method: 'PATCH', headers: { 'Content-Type': 'application/json', 'apikey': SUPA_KEY, 'Prefer': 'return=minimal' },
...
function timeAgo(d) {
  ...
  if (days===0) return 'Hoje'; if (days===1) return 'Ontem'; if (days<7) return `${days} dias atras`;
  return formatDate(d);
}
function initials(name) { if (!name) return '?'; return name.trim().split(' ').map(w=>w[0]).join('').slice(0,2).toUpperCase(); }
function statusLabel(s) {
  return {sent:'Aguardando',approved:'Aprovado',declined:'Recusado',expired:'Expirado',cancelled:'Cancelado'}[s]||s;
}
function requireAuth() { if (!API.token()) { window.location.href='/index.html'; return false; } return true; }
function logout() {
  fetch(`${SUPA_URL}/auth/v1/logout`,{method:'POST',headers:{'Content-Type':'application/json','apikey':SUPA_KEY,'Authorization':`Bearer ${API.token()}`}}).catch(()=>{});
